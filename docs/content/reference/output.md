...
describes what users get. Pick a format with `-o`, or let jutge choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
jutge <command> -o table   # aligned columns for reading
jutge <command> -o jsonl   # one JSON object per line, for piping
jutge <command> -o json    # a single JSON array
jutge <command> -o csv     # spreadsheet friendly
jutge <command> -o tsv     # tab-separated
jutge <command> -o url     # just the URL column
jutge <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
jutge <command> --fields id,title,url
```
...
```bash
jutge <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
jutge <command>            # a table, because this is a terminal
jutge <command> | wc -l    # JSONL, because this is a pipe
```
